...
export default function AdminSettings() {
	const [coins, setCoins] = useState([{ name: "", address: "", network: "", price: 0 }]);
	...
	const handleCoinChange = (index: number, field: string, value: string | number) => {
	...
	return (
		...
		>
			<div className="flex items-start justify-between p-4 border-b rounded-t dark:border-gray-900">
				<h3 className="text-xl font-semibold text-gray-900 dark:text-white">Edit Utility Details</h3>
			</div>
			<div className="p-6 space-y-6">
				{coins.map((coin, index) => (
					<div key={index} className="grid grid-cols-6 gap-6">
						...
						<div className="col-span-6 sm:col-span-3">
							<label htmlFor={`coin-address-${index}`} className="editUserLabel">
								Address
							</label>
							<input
								value={coin.address}
						...
						<div className="col-span-6 sm:col-span-3">
							<label htmlFor={`coin-network-${index}`} className="editUserLabel">
								Network
							</label>
							<input
								value={coin.network}
						...
						<div className="col-span-6 sm:col-span-3">
							...
							<input
								value={coin.price}
								onChange={(e) => handleCoinChange(index, "price", parseFloat(e.target.value))}
								type="number"
								id={`coin-price-${index}`}
								className="editUserInput"
								required
				...
				{success && <Alert type="success" message={"Wallet Updated successfully"} />}
			</div>
