...
export default function ApprovedDeposits() {
	...
	const fetchDeposits = async () => {
		try {
			...
			if (res.ok) setDeposits(data.filter((dep: any) => dep.status === "approved"));
	...
	return (
		<div className="relative overflow-x-auto rounded-[6px]">
			<table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-700">
				...
				<tbody>
					{deposits.length > 0 &&
						deposits.map((deposit: ITransaction, i: number) => (
							<tr className="bg-white dark:bg-gray-800" key={i}>
								...
								>
									<img
										...
										src={`https://robohash.org/${deposit.user.id}`}
										alt="Avatar"
									/>
									<div className="ps-3">
								...
								>
									<div className="text-xs font-semibold">{deposit.walletData.coinName}</div>
									<div className="text-xs font-medium text-gray-500">
										{deposit.walletData.convertedAmount}
									</div>
								</td>
								<td className="px-6 py-4 max-sm:text-[10px] min-w-28">{deposit.date.slice(0, 10)}</td>
